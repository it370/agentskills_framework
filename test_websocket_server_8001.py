"""
WebSocket Test Server on Port 8001
Run this alongside the REST server on port 8000
"""
import asyncio
import websockets
...
async def handle_client(websocket, path):
    ...
    try:
        ...
        welcome = {
            "type": "connected",
            "message": "Port 8001 WebSocket is accessible!",
...
async def main():
    ...
    print("=" * 70)
    print(f"🚀 WebSocket Test Server Running on Port {port}")
    print("=" * 70)
    print(f"   Listening on: {host}:{port}")
    print(f"   Local test:   ws://localhost:{port}/")
    print(f"   External:     ws://YOUR_VM_IP:{port}/")
    print()
    print("   To test: Open http://localhost:8000/test in your browser")
    print()
